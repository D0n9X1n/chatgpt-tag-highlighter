...
- Delete untagged chats (`showDeleteUntagged`, default off) uses ChatGPT's private endpoints: `GET /api/auth/session` → `accessToken`; `GET /backend-api/conversations?offset=&limit=&order=updated` (page by items received until an empty page — `total` is not a reliable count, and the server may cap `limit`); `PATCH /backend-api/conversation/{id}` with `{"is_visible": false}`, which is what ChatGPT's own Delete sends. If a chat appears twice while paging, merge copies so any tagged/protected copy wins; the list is re-fetched at confirm time and only chats still untagged are deleted. Requests go through `pageFetch` (Firefox: `content.fetch` + `cloneInto`, untested at runtime). Tests mock these routes. Never run the delete path against a real account; to probe the endpoint, PATCH a random UUID and expect `404 conversation_not_found`.
